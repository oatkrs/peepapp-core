...
peepapp_LOGIN_URI = '/api/get_auth_token/'


class peepappConnector:
    """
    PeepApp connector helps you to interact with it.
    Usage:
    >>> peepapp = peepappConnector("http://localhost:8000", "/api/report/1/")
    >>> peepapp.login("user", "password")
    >>> peepapp.get_report_info()
    >>> peepapp.download_apk("/tmp")
    >>> peepapp.upload_pcap("/tmp/my_file.pcap")
    """
    def __init__(self, host, report_info_uri):
        """
        Instantiate a new Peepapp connector.
        ``report_info_uri`` is displayed on top of each report
        when you are connected with a superuser account.
        :param host: Peepapp host e.g. http://localhost:8000
    ...
    def login(self, username, password):
        ...
        """
        r = requests.post(
            '{}{}'.format(self.host, peepapp_LOGIN_URI),
    ...
    def upload_pcap(self, pcap_file):
        ...
        Once uploaded, Peepapp will automatically analyze it.
    ...
    def upload_flow(self, flow_file):
        ...
        Once uploaded, Peepapp will automatically analyze it (not implemented yet).
